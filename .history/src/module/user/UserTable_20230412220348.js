...
const UserTable = () => {
      ...
      <tr key={user.id}>
        ...
        <th>
          <div className="flex gap-5 text-gray-400">
            ...
            ></ActionEdit>
            <ActionDelete onClick={() => handleDeleteUser()}></ActionDelete>
          </div>
        </th>
      </tr>
    );
  ...
  useEffect(() => {
    async function fetch() {
      ...
      setUserCurrent(results);
    }
    fetch();
  }, [userInfo.email]);
  const userNew = userCurrent[0];
  console.log(userNew.role);
  const handleDeleteUser = async (userNew) => {
    if (Number(userNew.role) !== userRole.ADMIN) {
      Swal.fire("Failed", "You have no right to do this action", "warning");
      return;
    }
    const colRef = doc(db, "users", userNew.id);
    Swal.fire({
      title: "Are you sure?",
